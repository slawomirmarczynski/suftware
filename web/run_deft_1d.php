...
$command = 'cat ' . $data_file_name . ' | ../code/deft_1d.py';

if (array_key_exists('alpha', $_POST))
    $command .= " --alpha=" . sanitizeString($_POST['alpha']);
...
if (array_key_exists('max_posterior_samples', $_POST)) {
    $command .= " --num_samples=" . sanitizeString($_POST['max_posterior_samples']);
}

$command .= ' --json';

// Save last command in file. Useful for debugging purposes
$myfile = fopen("commandline/last_deft_1d_command.sh","w");
